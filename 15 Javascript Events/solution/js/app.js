...
var thumbnailGallery = document.querySelector('#thumbnailGallery');

loadMember(0);

for (var i = 0; i < members.length; i++) {
    var memberImage = new Image();
    memberImage.src = members[i].src;
    memberImage.alt = "Profile picture for " + members[i].name;
    memberImage.className = "thumbnailPics";
    memberImage.id = i;
    thumbnailGallery.appendChild(memberImage);
    memberImage.addEventListener("click",function() { loadMember(this.id)}); 
}

function loadMember(i) {
    var memberImage = document.querySelector('article > img');
    var h3 = document.querySelector('article > h3');
    var para = document.querySelector('article > p');
    
    memberImage.src = members[i].src;
    memberImage.alt = "Profile picture for " + members[i].name;
    
    h3.innerText = members[i].name;
    
    para.innerText = members[i].bio;
}
...
h1.style.backgroundImage = "url(" + images[currentPosition] + ")";
